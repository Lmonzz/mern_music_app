...
const Login = ({ setAuth }) => {
  ...
  //lay authencation ma ta enable cho project 
  const firebaseAuth = getAuth(app);
  const provider = new GoogleAuthProvider();

  //khi users login thành công, ta cần navigate user đến homepage (navigate option)
  const navigate = useNavigate();

  const [{user}, dispatch] = useStateValue()

  const loginWithGoogle = async () => {
    //get firebase app và một số thứ (truoc tien import app)
